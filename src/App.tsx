...
import saveBiteBanner from '../ref_video/savebite.png';
import roamoraBanner from '../ref_video/roamora.png';
import nomadCoastBanner from '../ref_video/nomad_coast.png';
import taskNestBanner from '../ref_video/tasknest.png';
import governmentServicesBanner from '../ref_video/government_services.png';
import smaWebsiteBanner from '../ref_video/website.png';
import smaDashboardBanner from '../ref_video/dashboard.png';
...
const work:Project[]=[
  {name:'SaveBite',banners:[saveBiteBanner],category:'Surplus-Food Marketplace',type:'app',summary:'A mobile marketplace that helps people rescue quality surplus food from nearby kitchens, bakeries and grocers.',detail:'Designed around fast discovery, simple pickup scheduling and visible impact tracking, SaveBite turns everyday food choices into less waste and a greener community.',playStore:'https://play.google.com/store/apps/details?id=com.wajba.app&hl=en',appStore:'https://apps.apple.com/us/app/wajba-save-fresh-meals/id6740336691'},
  {name:'Roamora',banners:[roamoraBanner],category:'Modern Coastal Tourism App',type:'app',summary:'A vibrant coastal companion for discovering events, attractions, beaches, hotels and curated local experiences.',detail:'Roamora combines destination search, map-based exploration and ready-made itineraries in a polished experience built to make every coastal trip effortless.',playStore:'https://play.google.com/store/apps/details?id=com.btea.tg&hl=en',appStore:'https://apps.apple.com/us/app/visit-bahrain/id1136770500'},
  {name:'Nomad Coast',banners:[nomadCoastBanner],category:'Premium Travel & Itinerary App',type:'app',summary:'A premium travel experience for exploring dramatic coastlines, cultural events and memorable escapes.',detail:'Editorial destination discovery, interactive maps and a flexible day-by-day itinerary builder help travelers shape a refined journey from inspiration to arrival.',playStore:'https://play.google.com/store/apps/details?id=com.btea.tg&hl=en',appStore:'https://apps.apple.com/us/app/visit-bahrain/id1136770500'},
  {name:'TaskNest',banners:[taskNestBanner],category:'On-Demand Home Services',type:'app',summary:'A friendly home-services platform that connects customers with trusted professionals in minutes.',detail:'TaskNest simplifies service selection, scheduling and booking confirmation through a calm, guided flow designed to give people more time for what matters.',playStore:'https://play.google.com/store/apps/details?id=com.faz3a.client&hl=en',appStore:'https://apps.apple.com/us/app/faz3a-%D9%81%D8%B2%D8%B9%D8%A9/id6469709038'},
  {name:'NationConnect',banners:[governmentServicesBanner],category:'Nationwide Government Services App',type:'app',summary:'A modern nationwide civic platform that brings public information, local updates and essential services into one accessible app.',detail:'Clear navigation, nearby-service discovery, community news and emergency assistance make everyday government interactions simpler and more transparent across the country.',playStore:'https://play.google.com/store/apps/details?id=com.klabs.southerngovernorate&hl=en',appStore:'https://apps.apple.com/us/app/aljunobya/id1480147210'},
  {name:'SMA Collections',banners:[smaWebsiteBanner,smaDashboardBanner],category:'E-Commerce Website & Admin Dashboard',type:'web',summary:'A premium online storefront for discovering and shopping curated watches and footwear.',detail:'A complete e-commerce platform featuring a responsive customer storefront and an administration dashboard for managing products, inventory, orders, customers and analytics.',website:'https://customer-store.customer-store.workers.dev/'}
];
const skills=[
...
function ProjectArtwork({project,index}:{project:Project;index:number}){
  return <div className={`project-art art${index+1} is-loaded`}>
    <div className="project-banner-link" aria-label={`${project.name} preview`}>
      <div className={`project-banner-track ${project.banners.length>1?'is-gallery':''}`}>
        {project.banners.map((banner,bannerIndex)=><img src={banner} alt={`${project.name} project preview`} key={`${banner}-${bannerIndex}`}/>) }
      </div>
    </div>
  </div>;
}
